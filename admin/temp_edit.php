  ...
  <body>
    ...
    if ($_POST) {
     if (empty($_POST['date']) || empty($_POST['vr_no']) || empty($_POST['supplier_id']) || empty($_POST['item_id']) || empty($_POST['price']) || empty($_POST['qty'])) {
       ...
       $stmt = $pdo->prepare("UPDATE temp SET date=:date, vr_no=:vr_no, supplier_id=:supplier_id, item_id=:item_id, price=:price, qty=:qty WHERE id='$id'");
       $result = $stmt->execute(
         array(':date'=>$date,':vr_no'=>$vr_no,':supplier_id'=>$supplier_id,':item_id'=>$item_id,':price'=>$price,':qty'=>$qty)
       );
       if ($result) {
         echo "<script>alert('Your Update is Successfull');window.location.href='temp.php';</script>";
       }
     }
    }

    $stmt = $pdo->prepare("SELECT * FROM temp WHERE id=".$_GET['id']);
    $stmt->execute();
